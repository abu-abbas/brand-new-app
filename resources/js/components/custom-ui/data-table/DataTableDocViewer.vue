...
import { BookOpen, Check, Copy, List, CheckSquare, Square } from '@lucide/vue';
...
<template>
  <div class="flex flex-col gap-6">
    <!-- Card Header & Quick Table of Contents -->
    <Card class="border-primary/20 bg-card shadow-xs">
      ...
      <CardContent v-if="parsedDoc.toc.length > 0">
        <div
          class="flex items-center gap-2 mb-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider"
        >
          <List class="size-3.5" />
        ...
        <div class="flex flex-wrap gap-1.5">
          ...
          >
            <span v-if="item.level === 3" class="text-muted-foreground text-[10px]">&bull;</span>
            {{ item.title }}
          </Button>
        </div>
      </CardContent>
          ...
          <ul v-else-if="node.type === 'list'" class="space-y-1.5 my-2 pl-2">
            ...
            >
              <template v-if="item.checked !== undefined">
                <CheckSquare v-if="item.checked" class="size-4 text-emerald-500 shrink-0 mt-0.5" />
