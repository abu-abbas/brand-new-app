...
4. _Catatan Penting_: Otorisasi frontend adalah bagian dari kenyamanan UX. Backend tetap bertindak sebagai garda keamanan utama (source of truth).
...
2. **Aksi Hapus**: Penghapusan data wajib memanggil `await useConfirmDialog()({ ... })` dengan `confirmVariant: 'destructive'`. Letakkan mutation pada `onConfirm` agar tombol menampilkan loader selama proses.
...
- [ ] Penghapusan data meminta konfirmasi melalui `useConfirmDialog()` dengan variant destructive.
